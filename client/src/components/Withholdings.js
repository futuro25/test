  ...
  return (
    <div className="h-full overflow-auto">
      <div className="w-full flex sticky top-0 z-10 bg-white shadow px-4 pb-4">
        <h1 className="inline-block text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight ">Retenciones</h1>
      ...
      }
      {
        stage === 'LIST' && data && (
          <div className="mt-4 -mb-3 px-4">
            <p className="pl-1 pb-1 text-slate-500">Total de retenciones {data.length}</p>
                ...
                <div className="shadow-sm overflow-auto my-8">
                  <table className="border-collapse table-auto w-full text-sm">
                    ...
                    <tbody className="bg-white ">
                      {
                        dataFiltered.length ? 
                          dataFiltered.map((withholding, index) => (
                            <tr key={withholding._id} className={utils.cn('border-b last:border-b-0 hover:bg-gray-100', (index % 2 === 0) && 'bg-gray-50')}>
                              <td className="text-left !text-xs border-b border-slate-100  p-4 pl-8 text-slate-500 ">{upperCase(withholding.name)}</td>
                        ...
                        <tr>
                          <td>
                            <div className="p-4 gap-2 flex items-center">
                              ...
                              {
                                viewOnly ? 
                                <label className="text-slate-500 w-20">{selectedWithholding?.name}</label>
                                :
                                <input onKeyDown={utils.handleKeyPress} type="text" defaultValue={selectedWithholding?.name || ''} {...register("name", { required: true })} className="rounded border border-slate-200  p-4 pl-8 text-slate-500 " />
                              }
